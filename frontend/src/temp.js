...
import { Calendar, CheckCircle, Circle, Plus, HeartPulse, Target, Clock, Bell, Link, Edit, Trash2, Star, Zap, Moon, Sun } from 'lucide-react';
import AuthPage from './AuthPage';
import problemsData from './problems.json';

// =================================================================================
// Constants & Utilities (No changes needed here)
// =================================================================================
...
const getDifficultyColor = (difficulty, isDarkMode) => {
    const baseColors = {
      'Easy': isDarkMode ? 'text-green-400 bg-green-900/50' : 'text-green-600 bg-green-100',
      'Medium': isDarkMode ? 'text-yellow-400 bg-yellow-900/50' : 'text-yellow-600 bg-yellow-100',
      'Hard': isDarkMode ? 'text-red-400 bg-red-900/50' : 'text-red-600 bg-red-100',
...
const formatNoteContent = (content) => {
    ...
    return parts.map((part, index) => {
        if (part.startsWith('```') && part.endsWith('```')) {
            ...
            return (
                <div key={index} className="my-3">
                    <div className="bg-gray-800 text-gray-200 px-3 py-1 text-xs font-mono rounded-t-md border-b border-gray-600">{language}</div>
        ...
        return part.split('\n').map((line, i) => <div key={i}>{line}{i < part.split('\n').length - 1 && <br />}</div>);
    });
};
...
// =================================================================================
// Child Components (Paste your original code inside these components)
// =================================================================================

const ProblemCard = ({ problem, note, onToggle, onSaveNote, isDarkMode }) => {
...
const TaskManager = ({ studyPlan, isDarkMode, notes, addCustomProblem, toggleProblemStatus, saveNote }) => {
...
const Dashboard = ({ problems, spacedRepetition, isDarkMode }) => {
...
const SetupForm = ({ formData, setFormData, handleFormSubmit, isDarkMode }) => {
...
// =================================================================================
// Main Parent Component: CodePulseTracker
// =================================================================================

const CodePulseTracker = () => {
    ...
    const [formData, setFormData] = useState({ level: 'beginner', days: 30, topics: [] });

    useEffect(() => {
        document.title = 'CodePulse - Personalized Coding Progress Tracker';
    ...
    const handleLogout = () => { setUser(null); setIsAuthenticated(false); setStudyPlan(null); setProblems([]); setNotes({}); setActiveTab('setup'); };
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                {!studyPlan && activeTab !== 'setup' && (
                     <div className="text-center py-12">
                         <Zap className={`mx-auto mb-4 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} size={48} />
                         <h3 className={`text-lg font-medium mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>No Progress Plan Created</h3>
                         <p className={`mb-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Create a progress plan first to access this feature.</p>
                         <button onClick={() => setActiveTab('setup')} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                             Create a Plan
                         </button>
                     </div>
                )}
...
export default CodePulseTracker;