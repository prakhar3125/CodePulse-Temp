...
import { Calendar, CheckCircle, Circle, Plus, BookOpen, Target, Clock, Bell, Link, Edit, Trash2, Star, Brain } from 'lucide-react';
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-sm text-gray-600 mt-2">{stats.completed} of {stats.total} problems completed</p>
        </div>
      </div>
    );
  };
  ...
  const getStatusIcon = (status) => {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Task Manager - Day by Day</h2>
      ...
      {showCustomForm && (
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-lg font-semibold mb-4">Add Custom Problem</h3>
      ...
      <div className="space-y-4">
        {[...(customProblemsDay ? [customProblemsDay] : []), ...dailyPlans].map(dayPlan => {
          ...
          return (
            <div 
              key={dayPlan.day} 
              className={`bg-white rounded-lg shadow border-l-4 transition-all ${getStatusColor(dayStatus)}`}
            >
              {/* Day Header */}
              <div 
    ...
    <div className="border rounded-lg p-4 hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className={`font-medium ${problem.status === 'completed' ? 'line-through text-gray-500' : 'text-gray-800'}`}>
              {problem.name}
            </h4>
            <p className="text-sm text-gray-600">{problem.topic}</p>
          </div>
        </div>
      ...
      {notesMode === 'view' && hasNotes && (
        <div className="mt-3 p-4 bg-blue-50 rounded-lg border-l-4 border-blue-500">
          <div className="flex justify-between items-start mb-2">
            <h5 className="font-medium text-blue-900">Notes:</h5>
            <button
              onClick={() => {
                setNoteText(notes[problem.id]);
                setNotesMode('edit');
        ...
        <div className="mt-3 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="mb-3 p-3 bg-white rounded-lg border">
            <div className="flex flex-wrap gap-2 items-center">
              <span className="text-sm font-medium text-gray-700">Select Sanguage:</span>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {!studyPlan && activeTab !== 'setup' && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No Study Plan Created</h3>
            <p className="text-gray-600 mb-4">Create a study plan first to access this feature.</p>
            <button
